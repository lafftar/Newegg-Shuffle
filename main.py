...
class Newegg:
    def __init__(self):
        ...
        if settings["captcha"]["captcha_solver"] == 1 or str(
                settings["captcha"]["captcha_solver"]).lower() == "capmonster":
            api_key = settings["captcha"]["keys"]["capmonster"]
        elif settings["captcha"]["captcha_solver"] == 2 or str(
                settings["captcha"]["captcha_solver"]).lower() == "anticaptcha":
            api_key = settings["captcha"]["keys"]["anticaptcha"]
        elif settings["captcha"]["captcha_solver"] == 3 or str(
                settings["captcha"]["captcha_solver"]).lower() == "2captcha":
    ...
    def load_settings(self):
        ...
        self.PATH_EMAILS = f'{get_project_root()}/data/emails.txt'
        self.PATH_PROXIES = f'{get_project_root()}/data/proxies.txt'
        if platform.system() == "Linux" or platform.system() == "Darwin":
            self.PATH_EMAILS = self.PATH_EMAILS.replace("\\", "/")
            self.PATH_PROXIES = self.PATH_PROXIES.replace("\\", "/")

        # Open settings file
        settings_file_path = f'{get_project_root()}/data/settings.json'
    ...
    def submit_entries(self):
        ...
        index = 0
        for item in items:
            print(Fore.GREEN + str(index), Fore.MAGENTA + "=",
                  Fore.CYAN + "{} (${})".format(item["name"], item["price"]))
            index += 1
        ...
        choices = input(
            Fore.MAGENTA + "Enter the numbers of all the products you'd like to enter for, seperated by a space. (Example: 1 5 7 9 ): " + Fore.GREEN)
